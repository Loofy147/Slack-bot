...
async def list_projects(
    ...
    async with db_pool.acquire() as conn:
        ...
        query += " ORDER BY p.created_at DESC LIMIT $%d OFFSET $%d" % (len(params) + 1, len(params) + 2)
...
async def start_orchestration(
    ...
    async with db_pool.acquire() as conn:
        ...
        background_tasks.add_task(
            ...
            run_id, request.topic, request.integration_enabled, user
        )
...
async def list_templates(
    ...
    async with db_pool.acquire() as conn:
        ...
        if is_public is not None:
            query += " AND t.is_public = $%d" % (len(params) + 1)
    ...
    async with db_pool.acquire() as conn:
        ...
        runs_summary = await conn.fetchrow(
            """SELECT 
               ...
               WHERE user_id = $1 AND created_at >= NOW() - INTERVAL '%d days'""" % days,
            user["id"]
        )
...
# Background Tasks
async def run_orchestration_background(run_id: uuid.UUID, topic: str, integration_enabled: bool, user: dict):
    ...
    try:
        ...
        results = orchestrator.process_topic_enhanced(topic, integration_enabled)
        ...
        logger.error(f"Orchestration failed for run {run_id}: {e}")
        async with db_pool.acquire() as conn:
            await conn.execute(
                """UPDATE orchestration_runs 
