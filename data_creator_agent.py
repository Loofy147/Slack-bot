"""
This module defines the DataCreatorAgent class and its dependencies.
"""
import asyncio
import os
import logging
    ...
    def send(self, message, target_agent_name=None):
        ...
        if self.message_bus:
            if target_agent_name:
                print(
                    f"Agent '{self.name}' sending direct message to '{target_agent_name}' via Message Bus.")
                # Use asyncio.create_task to send without blocking the agent's main loop
                asyncio.create_task(self.message_bus.publish(
                    message, target=target_agent_name))
            elif message.get('type'):
                print(
                    f"Agent '{self.name}' publishing event of type '{message.get('type')}' via Message Bus.")
                asyncio.create_task(self.message_bus.publish(message))
            else:
                print(
                    f"Agent '{self.name}' Warning: Cannot send message. No target agent specified and message is missing 'type' field.")
        else:
            print(
                f"Agent '{self.name}' Warning: Message bus not configured. Message not sent.")

    async def run(self):
        """The main run loop for the agent."""
        print(
            f"Agent '{self.name}' starting run loop. Current State: {self.state}")
        self.state = "running"
        try:
            while True:
                if self.inbox:
                    ...
                    print(
                        f"Agent '{self.name}' picking up message from internal inbox. Current State: {self.state}")
            ...
            self.state = "stopped"
            print(
                f"Agent '{self.name}' run loop finished. Final State: {self.state}")

    async def handle_message(self, msg):
        """Handles a message from the inbox."""
        print(
            f"Agent '{self.name}' handling message: {msg}. Current State: {self.state}")
        print(
            f"Agent '{self.name}' processed unsupported message type or is in an incompatible state.")


# Ensure CentralMessageBus is defined
class CentralMessageBus:
    """A central message bus for inter-agent communication."""
    ...
    def register_agent(self, agent_name, inbox_ref):
        """Registers an agent's inbox with the message bus."""
        if agent_name in self._agent_inboxes:
            print(
                f"Warning: Agent '{agent_name}' inbox already registered. Overwriting registration.")
    ...
    def subscribe(self, event_type, agent):
        ...
        if agent not in self._subscriptions[event_type]:
            ...
            print(
                f"Message Bus: Agent '{agent.name}' subscribed to event type: {event_type}")
        else:
            print(
                f"Message Bus Warning: Agent '{agent.name}' is already subscribed to event type: {event_type}.")
    ...
    def unsubscribe(self, event_type, agent):
        ...
        if event_type in self._subscriptions and agent in self._subscriptions[event_type]:
            ...
            print(
                f"Message Bus: Agent '{agent.name}' unsubscribed from event type: {event_type}")
    ...
    async def publish(self, message, target=None):
        ...
        if target:
            if target in self._agent_inboxes:
                ...
                print(
                    f"Message Bus Publish Warning: Target agent '{target}' not found in _agent_inboxes. Direct message not delivered.")
        elif event_type:
            if event_type in self._subscriptions:
                for agent in list(self._subscriptions[event_type]):
                    if hasattr(agent, 'inbox'):
                        ...
                        print(
                            f"Message Bus Publish (Event) Warning: Agent {agent.name} has no inbox to deliver message.")
        else:
            print(
                "Message Bus Publish Warning: Cannot publish message. No target agent specified and message is missing 'type' field.")


# Ensure ArchitectureEventType is defined with all necessary types
class ArchitectureEventType:
    """Enum for architecture event types."""
    ...
    # From IngestionAgent
    INGESTED_DATA_BATCH = "ingested_data_batch"
    GENERATED_CODE = "generated_code"  # From CodeGeneratorAgent
    CODE_FOR_ANALYSIS = "code_for_analysis"  # From CodeGeneratorAgent
    # New type for code analysis results
    ANALYZED_CODE_STRUCTURE = "analyzed_code_structure"
    # New type for doc analysis results
    ANALYZED_DOCUMENTATION = "analyzed_documentation"
    AGENT_ANALYSIS_REPORT = "agent_analysis_report"  # New type for findings
    # New type for suggestions
    IMPROVEMENT_SUGGESTION = "improvement_suggestion"
    DATA_EMBEDDING_COMPLETE = "data_embedding_complete"
    TRAINING_PAIR_GENERATED = "training_pair_generated"


# Ensure extract_repo_data function is defined
def extract_repo_data(repo_path, include_patterns=None, exclude_patterns=None):
    """
    ...
    if not os.path.isdir(repo_path):
        print(
            f"Error: Repository path '{repo_path}' is not a valid directory.")
    ...
    print(
        f"Finished extraction from {repo_path}. Extracted {len(extracted_data)} files.")
    return extracted_data


# Define the DataCreatorAgent class
class DataCreatorAgent(Agent):
    """Agent for creating data from various sources."""
    ...
    def __init__(self, name="DataCreatorAgent", message_bus=None, config=None):
        ...
        if self.message_bus:
            self.message_bus.subscribe(
                ArchitectureEventType.USER_FEEDBACK, self)
    ...
    async def handle_message(self, msg):
        ...
        print(
            f"DataCreatorAgent '{self.name}' handling message: {msg}. Current State: {self.state}")

        msg_type = msg.get("type")
        msg_id = msg.get(
            "id", f"{msg_type}-{int(time.time())}-{random.randint(1000, 9999)}")
        ...
        if msg_type == "clone_and_extract_repo":
            ...
            if repo_url in self._processed_repo_tasks:
                print(
                    f"DataCreatorAgent: Skipping already processed repository '{repo_url}'.")
                return
        ...
        if msg_type == ArchitectureEventType.USER_FEEDBACK:
            if msg_id in self._processed_feedback_ids:
                print(
                    f"DataCreatorAgent: Skipping already processed feedback with ID {msg_id}.")
                return
            self._processed_feedback_ids.add(msg_id)

        # Main task handling logic
        supported_types = [ArchitectureEventType.USER_FEEDBACK,
                           "clone_and_extract_repo"]
        if self.state == "waiting_for_task" and msg_type in supported_types:
            ...
            try:
                ...
                logging.error(
                    "DataCreatorAgent Error processing message ID %s: %s", msg_id, e, exc_info=True)
                self.send({"type": "error", "details": f"Error processing message ID {msg_id}: {e}",
                          "original_message": msg}, "Orchestrator")
            finally:
                self.state = "waiting_for_task"
        else:
            print(
                f"DataCreatorAgent '{self.name}': Ignoring message type '{msg_type}' in state '{self.state}'.")
    ...
    async def process_user_feedback(self, feedback_msg):
        ...
        for category, pattern in self._feedback_parsing_patterns.items():
            ...
            if match:
                ...
                print(
                    f"DataCreatorAgent: Parsed feedback as '{category}'. Publishing structured event.")
        ...
        if not parsed:
            print(
                "DataCreatorAgent: Feedback did not match any known patterns. Forwarding as generic text.")
    ...
    async def process_repo_clone_and_extract(self, msg):
        ...
        if not repo_url:
            print(
                "DataCreatorAgent Error: 'clone_and_extract_repo' message missing 'repo_url'.")
            self.send({"type": "error", "details": "Missing 'repo_url'",
                      "original_message": msg}, "Orchestrator")
            return
        ...
        print(
            f"DataCreatorAgent: Starting clone for '{repo_url}' into '{local_repo_path}'")
        ...
        try:
            ...
            if not clone_success:
                raise RuntimeError("Git clone command failed.")
            ...
            if not extracted_data:
                print(
                    f"DataCreatorAgent Warning: No data extracted from '{repo_url}'. Might be empty or have no matching files.")
                self.send({"type": "warning", "details": "No data extracted from repo",
                          "source_repo": repo_url}, "Orchestrator")
                return
            ...
            print(
                f"DataCreatorAgent: Publishing {len(extracted_data)} files from '{repo_url}' to the message bus.")
            ...
            self.send({"type": "error", "details": error_message,
                      "original_message": msg}, "Orchestrator")
        except Exception as e:
            ...
            if os.path.exists(local_repo_path):
                print(
                    f"DataCreatorAgent: Cleaning up by removing '{local_repo_path}'.")
                try:
                    shutil.rmtree(local_repo_path)
                except OSError as e:
                    print(
                        f"DataCreatorAgent Cleanup Error: Failed to remove directory {local_repo_path}: {e}")
...
# ==============================================================================
async def main():
    """Main function to run the data creator agent."""
    ...
    # 2. Initialize Ag
    data_creator = DataCreatorAgent(message_bus=bus)

    # 3. Start the agent
    asyncio.create_task(data_creator.run())

    # 4. Simulate a message to the agent
    ...
    # 5. Keep the main thread alive to let the agent work
