...
class VectorDBStorageAgent(Agent):
    ...
    async def handle_message(self, msg):
        ...
        if msg.get("type") == ArchitectureEventType.DATA_EMBEDDING_COMPLETE:
            print(
                f"{self.name}: Received data embedding for "
                f"'{msg.get('original_payload').get('source_file')}'. Storing in vector DB...")

            try:
                self.store_in_vector_db(msg)
            except Exception as e:
                print(
                    f"Error storing data in vector DB for "
                    f"{msg.get('original_payload').get('source_file')}: {e}")
